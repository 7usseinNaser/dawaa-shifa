...
import { Bug, MessageSquare, Lightbulb, Hospital } from 'lucide-react'

export function FacilityDashboard({ profile }: { profile: Profile }) {
  const { t } = useLang()

  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      <h1 className="font-cairo font-bold text-xl">{t('dash.facility')}</h1>

      <div className="glass-card p-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-brand-green/20 flex items-center justify-center">
            <Hospital className="w-6 h-6 text-brand-green-light" />
          </div>
          <div>
            <p className="font-cairo font-bold text-lg">{profile.display_name}</p>
            <p className="text-sm text-[var(--text-muted)] font-tajawal">حساب مرفق طبي</p>
          </div>
        </div>
      </div>
